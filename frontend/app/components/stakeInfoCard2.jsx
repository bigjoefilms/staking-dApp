...
const LoadingPlaceholder = () => (
  <div className="text-center py-8">
    <p className="text-gray-500 text-lg">Loading staking information...</p>
...
const StakeInfoCard = () => {
    ...
    try {
      ...
      setStakeData((prev) => ({
        ...
        stakes: prev.stakes.map((stake) =>
          stake.id === stakeId ? { ...stake, accumulatedReward: 0 } : stake
        ),
      }));
    } catch (error) {
  ...
  const StakeCard = ({ stake }) => {
    const endTime = new Date(
      stake.startTime + stake.duration * 24 * 60 * 60 * 1000
    );
